...
void ItemTracker::updateStash(const Point &point) {
	...
	if (s != stashes[saiph->position.branch][saiph->position.level].end()) {
		...
		saiph->debugfile << "[ItemTracker] Stash at " << point.row << ", " << point.col << " is changed! symbol changes: " << s->second.top_symbol << " - " << symbol << endl;
...
void ItemTracker::addItemToInventory(unsigned char key, const Item &item) {
	if (item.count < 0)
		return;
	saiph->debugfile << "[ItemTracker] Adding " << item.count << " " << item.name << " to inventory slot " << key << endl;
...
void ItemTracker::addItemToPickup(unsigned char key, const Item &item) {
	if (item.count < 0)
		return;
	saiph->debugfile << "[ItemTracker] Adding " << item.count << " " << item.name << " to pickup slot " << key << endl;
	pickup[key] = item;
}
...
void ItemTracker::addItemToStash(const Coordinate &coordinate, const Item &item) {
	if (item.count < 0)
		return;
	saiph->debugfile << "[ItemTracker] Adding " << item.count << " " << item.name << " to stash at " << coordinate.branch << ", " << coordinate.level << ", " << coordinate.row << ", " << coordinate.col << endl;
...
void ItemTracker::clearStash(const Coordinate &coordinate) {
	...
	saiph->debugfile << "[ItemTracker] Clearing stash at " << coordinate.branch << ", " << coordinate.level << ", " << coordinate.row << ", " << coordinate.col << endl;
	map<Point, Stash>::iterator s = stashes[saiph->position.branch][saiph->position.level].find(coordinate);
	if (s != stashes[saiph->position.branch][saiph->position.level].end())
...
void ItemTracker::removeItemFromInventory(unsigned char key, const Item &item) {
	if (item.count < 0)
		return;
	saiph->debugfile << "[ItemTracker] Removing " << item.count << " " << item.name << " from inventory slot " << key << endl;
...
void ItemTracker::removeItemFromPickup(const Item &item) {
	...
	if (item.count < 0)
		return;
	saiph->debugfile << "[ItemTracker] Removing " << item.count << " " << item.name << " from pickup" << endl;
...
void ItemTracker::removeItemFromStash(const Coordinate &coordinate, const Item &item) {
	if (item.count < 0)
		return;
	saiph->debugfile << "[ItemTracker] Removing " << item.count << " " << item.name << " from stash at " << coordinate.branch << ", " << coordinate.level << ", " << coordinate.row << ", " << coordinate.col << endl;
	map<Point, Stash>::iterator s = stashes[saiph->position.branch][saiph->position.level].find(coordinate);
	if (s != stashes[saiph->position.branch][saiph->position.level].end())
